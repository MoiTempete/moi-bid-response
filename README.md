...
- 段前/段后间距全部归零（原 H1 标题 24pt → 0，H2-H5 标题 12pt → 0，正文 4pt → 0），消除意外空白
- 标题字体加粗（`style.font.bold = True`），H2-H5 统一使用黑体加粗
- 正文段落新增首行缩进 2 字符（`first_line_indent = 0.74cm`），贴合中文公文排版习惯
- 新增 `--json-file` 参数支持从文件读取 JSON（`python3 generate_docx.py out.docx data.json`），避免管道编码问题。原有 stdin 模式兼容保留
- 新增生成后字数自检：`analyze_word_count()` 逐章对比实际字数与 `word_target` 目标，偏差超过 40% 时输出警告并提示扩写/精简

**references/writing-guide.md 引用格式规范**

- 新增「引用与强调的标点约定」章节，按场景推荐标点优先级：`""` > `<>` > `《》` > `【】`
- 建议避免使用直角引号「」（docx 渲染中可能出现字体兼容问题）

**references/doc-generation-strategy.md 大文档生成指引（新增）**

- 文档化"分步构建 + 管道串联"策略：当响应 JSON 超过 ~50KB 时，拆分为多个 Python 脚本逐段追加章节
- 提供每步脚本模板、辅助函数定义、常见陷阱对照表、步骤数估算指南
- 新增「递归大纲」机制：单章字数 > 3,000 时，可对该章递归应用"生成子大纲 → 确认 → 逐节编写"流程，避免大章节结构扁平化

**memory/ 项目记忆（新增）**

- 沉淀中文弯引号在多条工具链路中的编码行为差异及安全策略
- 后续类似工作可复用分步构建模板，减少编码踩坑

## 关联 Skills

### 技术响应完成后 → 生成述标 PPT

`moi-bid-response` 完成技术响应文件输出后，会自动询问是否需要生成讲标答辩（述标）PPT。如需使用，推荐配套 skill：

**[`moi-bid-defense`](https://github.com/MoiTempete/moi-bid-defense)** — 从技术响应文件中自动提取差异化亮点和量化指标，生成带讲标备注和预判 Q&A 的幻灯片。
...
`moi-bid-defense` 会进一步引导安装 [`guizang-ppt`](https://github.com/op7418/guizang-ppt-skill) 作为 HTML 幻灯片渲染引擎（瑞士国际主义风 / 电子杂志风，单文件 HTML，浏览器即开即用）。
